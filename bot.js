	...
	if (command == prefix) //Entering Srs bot prefix code section
	{
		switch (args[0].toLowerCase())
		{
			...
			case "translate":
				...
				if (args[1] == "help")
				{
					let langMessage = new Discord.MessageEmbed(); //Creates a fancy embembed to warn them
					...
					langMessage.setDescription(`English: 'en' \n` +
						...
						`Polish: 'pl' \n` +
						`Russian: 'ru' \n` +
						`Japanese: 'ja' - not sure if this will work \n` +
						`Chinese: 'zh' `);
				...
				else
				{
					languageArray = [ //Makes sure the translation language is currently supported
						"en", "es", "de", "fr", "pt", "it", "nl", "pl", "ru", "ja", "zh"
					]
					...
					try
					{
						...
						translateRequest.open("GET", 
							`${ajaxAPI}https://www.deepl.com/en/translator#${languageArray[args[1]]}/${languageArray[args[2]]}/${translateString}`);
			...
			case "test": //Test random new code
				let dom = new Jsdom("", {
